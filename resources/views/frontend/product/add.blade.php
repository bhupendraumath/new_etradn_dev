                ...
                <div class="form-settings">
                    <form method="post" enctype="multipart/form-data" id="addProductFrm">
                        ...
                        <div id="drop_area" class="area">
                            <div>
                                <label for="uploadImage" class="btn uploadimage-product choose-drag-drop"><b>Choose or
                                        Drag & Drop
                                        <br /> Your Product Images Here! </b></label>
                                <input id="uploadImage" onChange="setImage(this,'profile_image');"
                                    accept="image/png,image/jpg,image/jpeg" style="visibility:hidden;" type="file">
                        ...
                        <label class=" left-align">Brand</label>
                                ...
                                <input type="text" name="product_name_eng" placeholder="Product Name in English*"
                                    class="60per" value="{{(isset($product->product_name))?$product->product_name:''}}">

                                <label class="left-align">Product Description</label>
                                <textarea name="product_description_eng" placeholder="Product Description English*"
                                    value="{{(isset($product->product_desc))?$product->product_desc:''}}">{{(isset($product->product_desc))?$product->product_desc:''}}</textarea>

                                <label class="left-align">Product Warranty</label>
                                <input type="text" name="warranty_description_eng"
                                    placeholder="Product warranty in English*" class="60per"
                                ...
                                <div class="change-position">
                                    ...
                                    <input type="radio" name="list_product" class="radio" value="a"
                                        {{(isset($product->want_to_list)&&$product->want_to_list=='a')? 'checked':''}}>
                                    ...
                                    <div class="list_productyes {{(isset($product->list_product)&&($product->list_product=='a' ||
                                        ...
                                        <input type="number" name="bid_amount" placeholder="Min Bid Amount"
                                            class="60per"
                                            value="{{(isset($product->bid_amount))?$product->bid_amount:''}}">

                                        <label class="left-align">Bid Ending Date and Time</label>
                                        <input type="datetime-local" name="bid_ending_date_time"
                                            placeholder="Bid Ending Date and Time" class="60per"
                                            value="{{(isset($product->bid_ending_date_time))?$product->bid_ending_date_time:''}}">
                                    </div>
                                ...
                                <div class="change-position">
                                    <input type="radio" name="refund_request" class="radio" value="y"
                                        {{(isset($product->refund_request)&&$product->refund_request=='y')? 'checked':''}}>
                                ...
                                <div class="refundyes"
                                    ...
                                    <input type="number" name="number_of_days" placeholder="Number of days"
                                        class="60per"
                                        value="{{(isset($product->number_of_days))?$product->number_of_days:''}}">

                                    <label class="left-align">Policy description</label>
                                    <input type="text" name="refund_policy_description" placeholder="Policy description"
                                        class="60per"
                                        value="{{(isset($product->policy_description))?$product->policy_description:''}}">
                                </div>
                                <br />
                                ...
                                <div class="change-position">
                                    ...
                                    <input type="radio" name="shipping_type" class="radio" value="p"
                                        {{(isset($product->shipping_type)&&$product->shipping_type=='p')? 'checked':''}}>
                                    <label for="p">Paid</label>
                                    <br> <br />
                                    <div class="shipping_typeyes"
                                        {{(isset($product->shipping_type)&&$product->shipping_type=='p')? '':'d-none'}}>
                                        <label class="left-align">shipping price for one quantity</label>
                                        <input type="number" name="shipping_price"
                                            placeholder="shipping price for one quantity" class="60per"
                                            value="{{(isset($product->shipping_price))?$product->shipping_price:''}}">
                                    </div>
                                    <br /><br /><br />
                                </div>
                                <!-- ccc -->
                                            ...
                                            <div class="form-group">
                                                <div class="input-group">
                                                    <div class="input-group-btn">
                                                        <button class="btn btn-success classhidebutton" type="button"
                                                            onclick="education_fields();" style='width:40px;'> <span
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                for (var i = 0; i < result.data.length; i++) {
                    template += '<option value="' + result.data[i].id + '">' + result.data[
